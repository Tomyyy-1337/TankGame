...
use crate::physics::{Force, Mass, Position, Rotation};
...
fn ui_example_system(mut contexts: EguiContexts, query: Query<(&Position, &Force, &Mass, &Rotation), With<Player>>) {
    egui::Window::new("Debug Tank Data").show(contexts.ctx_mut(), |ui| {
        for (position, force, mass, rotation) in query.iter() {
